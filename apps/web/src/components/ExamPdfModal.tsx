  ...
  <Document title={title} author={teacher}>
    <Page size="A4" style={pdfStyles.page}>
      ...
      <View style={pdfStyles.footer} fixed>
        <Text>
          {title}
          {version ? ` (Versão ${version})` : ''} · {subject}
        </Text>
...
export function ExamPdfModal({
  ...
  const allFilteredSelected =
    filtered.length > 0 && filtered.every((q: QuestionDto) => selected.has(q.id));
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Configure, selecione questões e baixe
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex flex-col lg:flex-row flex-1 min-h-0">
          ...
          <div className="w-full lg:w-72 flex-shrink-0 border-b lg:border-b-0 lg:border-r border-slate-200 dark:border-slate-800 p-5 space-y-4 overflow-y-auto">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Ex: 1ª Avaliação Bimestral"
            ...
            <div className="space-y-2 pt-1">
              ...
              <div className="flex items-center space-x-2">
                <input
                  ...
                  onChange={(e) => setMultipleVersions(e.target.checked)}
                  className="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500"
                />
                <label
                  htmlFor="multiVer"
                  className="text-xs font-medium text-slate-700 dark:text-slate-300 cursor-pointer flex items-center space-x-1"
                >
            ...
            {multipleVersions && (
              <div className="p-2.5 bg-amber-50 dark:bg-amber-950/30 rounded-lg border border-amber-100 dark:border-amber-900 text-xs text-amber-800 dark:text-amber-300">
                <strong>Versão A:</strong> Ordem original
                <br />
            ...
            <div className="bg-indigo-50 dark:bg-indigo-950/40 rounded-xl p-3 space-y-1 border border-indigo-100 dark:border-indigo-900">
              ...
              <p className="text-xs text-indigo-700 dark:text-indigo-400">
                Fáceis:{' '}
                {selectedQuestions.filter((q: QuestionDto) => q.difficulty === 'EASY').length} ·{' '}
                Médias:{' '}
                {selectedQuestions.filter((q: QuestionDto) => q.difficulty === 'MEDIUM').length} ·{' '}
                Difíceis:{' '}
                {selectedQuestions.filter((q: QuestionDto) => q.difficulty === 'HARD').length}
              </p>
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col min-h-0">
            ...
            <div className="px-5 py-3 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between flex-shrink-0">
              ...
              <button
                onClick={toggleAll}
                className="flex items-center space-x-1 text-xs font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-700"
              >
                {allFilteredSelected ? (
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              ...
              {filtered.map((q: QuestionDto) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`flex-shrink-0 w-5 h-5 rounded border-2 flex items-center justify-center mt-0.5 transition-colors ${isChecked ? 'bg-indigo-600 border-indigo-600' : 'border-slate-300 dark:border-slate-600'}`}
                    >
              ...
              <div className="flex items-center justify-between px-4 py-3 bg-slate-800 border-b border-slate-700 flex-shrink-0">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  >
                    {({ loading }) => (
                      <button
                        className="flex items-center space-x-1.5 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-sm font-medium transition-colors"
                        disabled={loading}
                      >
                        <Download className="h-4 w-4" />
                        <span>
                          {loading ? 'Gerando...' : multipleVersions ? 'Versão A' : 'Baixar PDF'}
                        </span>
                      </button>
                    )}
                  </PDFDownloadLink>
                  {multipleVersions && (
                    ...
                    >
                      {({ loading }) => (
                        <button
                          className="flex items-center space-x-1.5 px-3 py-1.5 bg-amber-600 hover:bg-amber-500 text-white rounded-lg text-sm font-medium transition-colors"
                          disabled={loading}
                        >
                  ...
                  {({ loading }) => (
                    <Button variant="primary" isLoading={loading}>
                      ...
                      {loading
                        ? 'Gerando...'
                        : multipleVersions
                          ? `Versão A (${selectedQuestions.length}q)`
                          : `Baixar PDF (${selectedQuestions.length}q)`}
                    </Button>
                  )}
                </PDFDownloadLink>

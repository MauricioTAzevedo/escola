...
export async function teacherRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/analytics', { preHandler: [requireRole(['TEACHER', 'ADMIN'])] }, async (request, reply) => {
  ...
  fastify.get('/export-csv', { preHandler: [requireRole(['TEACHER', 'ADMIN'])] }, async (request, reply) => {
    const { subjectId } = request.query as { subjectId?: string };
    ...
    reply.header('Content-Disposition', `attachment; filename="banco_questoes_${subject?.name || 'disciplina'}.csv"`);
